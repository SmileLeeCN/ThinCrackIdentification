...
def Pixel_MA(Mask,GT):
    tp1 = np.sum(np.logical_and(Mask == 1, GT == 1))
    fp1 = np.sum(np.logical_and(Mask == 1, GT != 1))
    fn1 = np.sum(np.logical_and(Mask != 1, GT == 1))

    tp0 = np.sum(np.logical_and(Mask == 0, GT == 0))
    fp0 = np.sum(np.logical_and(Mask == 0, GT != 0))
    fn0 = np.sum(np.logical_and(Mask != 0, GT == 0))
    return tp1, fp1, fn1, tp0, fp0, fn0
